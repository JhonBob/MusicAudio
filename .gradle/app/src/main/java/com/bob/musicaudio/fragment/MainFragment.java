...
public class MainFragment extends Fragment implements IConstants,UIManager.OnRefreshListener,
    ...
    public View onCreateView(LayoutInflater inflater,
        ...
        mDefaultAlbumIcon = BitmapFactory.decodeResource(
                getActivity().getResources(), R.drawable.img_album_background);

        mHeadIcon = (ImageView) v.findViewById(R.id.headicon_iv);
        mAlbum=(ImageView)v.findViewById(R.id.iv_music_ablum);
        mAlbumReflection=(ImageView)v.findViewById(R.id.iv_music_ablum_reflection);
    ...
    public void refreshUI(MusicInfo music){
        Bitmap bitmap = MusicUtils.getCachedArtwork(getActivity(), music.albumId,
                mDefaultAlbumIcon);
        mHeadIcon.setBackgroundDrawable(new BitmapDrawable(getActivity().getResources(), bitmap));

        Bitmap mp= BitmapUtil.getArtwork(getActivity(), music.songId, music.albumId, true, false);
        if(mp!=null){
            mAlbum.setImageBitmap(mp);
            mAlbumReflection.setImageBitmap(ImageUtil.createReflectionBitmapForSingle(mp));
        }else{
            mp= BitmapUtil.getDefaultArtwork(getActivity(),false);
            mAlbum.setImageBitmap(mp);
            mAlbumReflection.setImageBitmap(ImageUtil.createReflectionBitmapForSingle(mp));
        }
    }
